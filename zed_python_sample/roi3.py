...
YOLOの抽出結果に対してROIを行い深度情報により2値化を行う

*輪郭抽出はある程度うまく行っているのでroi3にて輪郭の縦横を求める処理を作る
"""
...
def threshold_otsu(gray, min_value=0, max_value=255):
    ...
    hist = [np.sum(gray == i) for i in range(255)]
    # plt.hist(hist)
    ...
    for th in range(255):
        ...
        if n2 == 0 : mu2 = 0
        else : mu2 = sum([i * hist[i] for i in range(th, 255)]) / n2
...
def main(argv):
    ...
    init.depth_minimum_distance = 0.10
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            for detection in detections:
                ...
                roi_depth_show_gray = cv2.cvtColor(np.nan_to_num(roi_depth_show, copy=False), cv2.COLOR_RGB2GRAY)
                ...
                for i in range(0, len(contours)):
                    if len(contours[i]) > 0:
                     ...
                     roi = cv2.drawContours(roi,[box],0,(0,0,255),2)
                     cv2.line(roi, (rec1[0], rec1[1]), (rec3[0], rec3[1]), (255, 255, 255), thickness=2, lineType=cv2.LINE_AA)
                     cv2.line(roi, (rec2[0], rec2[1]), (rec4[0], rec4[1]), (255, 255, 255), thickness=2, lineType=cv2.LINE_AA)

                     #cv2.polylines(roi, contours[i], True, (255, 255, 255), 5)

                image[y_coord:y_coord+y_extent,x_coord:x_coord+x_extent] = roi
